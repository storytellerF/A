...
class KotbaseTest : UsingContextTest() {
    ...
    fun `test kotbase order`() = runTest {
        ...
        collection.save("1", CommunityInfo.EMPTY.copy(hasPoster = true, id = 1))
        collection.save("2", CommunityInfo.EMPTY.copy(hasPoster = false, id = 2))
        collection.save("3", CommunityInfo.EMPTY.copy(hasPoster = true, id = 3))
        collection.save("4", CommunityInfo.EMPTY.copy(hasPoster = false, id = 4))
        val task = collection.observeData(listOf(DatabaseOrder.Desc("hasPoster"), DatabaseOrder.Desc("id")), 10, null) {
