...
- Detekt baselines are stored as `detekt-baseline.xml` in each affected module. Auto-correct is disabled so static checks do not rewrite source files; remove baseline entries only when the corresponding violations are intentionally fixed.
...
- `shared/src/wasmJsMain/resources/mlCrypto.mjs` is the source of the bridge for shared Wasm `@JsModule` imports. Both executable WebApp modules add this source directory to their resources so Webpack bundles it into each final browser package; do not add a custom task that copies it into a compiler output directory.
