...
- `TaskRecordType.TOPIC_MODERATION` advances through topics by primary key. It reviews unencrypted community and user-space topic trees plus rooms whose `communityId` is non-null, skips private/encrypted rooms, changes an unsafe topic author's status to `UserStatus.READ_ONLY`, and sends the author a notification-room message. Successful ACG awards also notify the topic author through the same system-user notification-room path.
