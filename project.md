...
- Each executable's Wasm resource source must include the `mlCrypto.mjs` bridge for shared Wasm `@JsModule` imports (`app/composeApp/src/wasmJsMain/resources` and `panel/composeApp/src/wasmJsMain/resources`). Kotlin/Wasm processes and Webpack-bundles these resources automatically; do not add a custom task that copies them into a compiler output directory.
