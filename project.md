...
- Worker schedulers select one top-level business object per invocation. Subscription delivery is the exception only at its internal fan-out boundary: it drains every subscriber page for the selected topic in that invocation, writes a durable `SubscriptionSentLog` after each successful subscriber send (keyed by the topic ID), and creates the topic's `TaskRecord` only after an empty subscriber page confirms completion. A later invocation therefore resumes after the last logged subscription when delivery was interrupted.
