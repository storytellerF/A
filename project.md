...
- The application-owned `mlCrypto.mjs` bridge for shared Wasm `@JsModule` imports belongs in `app/composeApp/src/wasmJsMain/resources`. Kotlin/Wasm processes and Webpack-bundles that resource automatically; do not add a custom task that copies it into a compiler output directory.
...
- `deploy/sample.env` and the `sample` Compose profile start the full local dependency stack plus `app-wasm`. It intentionally uses the named Elasticsearch certificate volume from the `elastic` profile instead of `certs_bind`, which is only for host-local development. `compose-service.sh` builds `:app:composeApp:wasmJsBrowserDistribution` before Compose startup and passes the configured HTTP/WebSocket URLs through `-Papp.server.url` and `-Papp.ws.server.url`; the Nginx profile serves that output with COOP/COEP headers.
