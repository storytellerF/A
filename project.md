...
- `app/core` shares JVM/Android actual sources by adding `src/jvmAndroidMain/kotlin` as a source directory of both `jvmMain` and `androidMain`; do not create a custom `jvmAndroidMain` source set with explicit `dependsOn` edges because it prevents Kotlin's default hierarchy template from being applied.
