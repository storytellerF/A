...
package com.storytellerf.a.panelcliappium
...
private const val INSTALL_DIR_PROPERTY = "panel.cli.app.install.dir"
private const val TRANSCRIPT_LIMIT = 24_000
private const val TERMINAL_COLUMNS = 120
private const val TERMINAL_ROWS = 40

internal class PanelCliAppiumTest {
    private val terminalFactory = PanelCliTerminalFactory()

    @Test
    internal fun `register and browse overview through installed panel cli`() {
        runBlockingE2eTest {
            runBackendTestEnvironment { ports ->
                coroutineScope {
                    val terminal = terminalFactory.start(ports, this)
                    try {
                        terminal.awaitText("PANEL CLI")
                        terminal.sendLine("1")
                        ...
                        terminal.sendLine("6")
                        terminal.awaitText("Fetching Overview...")
                        terminal.awaitText("=== Overview ===")
...
private class PanelCliTerminalFactory(private val ioDispatcher: CoroutineDispatcher = Dispatchers.IO) {
    suspend fun start(ports: BackendPorts, scope: CoroutineScope): PanelCliTerminal =
        PanelCliTerminal.start(ports, scope, ioDispatcher)
}

private class PanelCliTerminal private constructor(
    ...
    suspend fun awaitText(expected: String) {
        ...
        assertTrue(isFound, "Panel CLI did not render '$expected'.\n${recentTranscript()}")
    }
    ...
    private suspend fun waitForText(expected: String): Boolean {
        while (true) {
            ...
            checkNotNull(chunk) {
                "Panel CLI exited before rendering '$expected'.\n${recentTranscript()}"
            }
    ...
    suspend fun awaitExit() {
        ...
        assertTrue(hasExited, "Panel CLI did not exit after selecting 0.\n${recentTranscript()}")
    }
    ...
    companion object {
        suspend fun start(ports: BackendPorts, scope: CoroutineScope, ioDispatcher: CoroutineDispatcher): PanelCliTerminal {
            ...
            check(executable.isFile) { "Panel CLI executable not found: ${executable.absolutePath}" }
