...
suspend fun UserSessionManager.getUserSignInPass(authKey: AuthKey): SignResult<UserInfo> =
    prepareSignInFromPrivateKey(
    authKey,
) {
    getData()
}.mapResult { param ->
...
suspend fun UserSessionManager.getUserSignUpPass(authKey: AuthKey): SignResult<UserInfo> =
    prepareSignInFromPrivateKey(
    authKey,
) {
    getData()
