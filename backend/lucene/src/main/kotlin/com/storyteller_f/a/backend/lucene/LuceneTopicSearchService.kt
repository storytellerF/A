...
data class LuceneTopicDocument(val topicDocument: TopicDocument) : LuceneDocument {
    ...
    companion object : LuceneDocumentCompanion<TopicDocument> {
        override fun restore(id: PrimaryKey, document: Document): TopicDocument =
            TopicDocument(
            id,
...
class LuceneTopicSearchService(path: Path, isInMemory: Boolean = false) :
    ...
    private fun buildQuery(topicDocumentSearch: TopicDocumentSearch): Query =
    ...
    private fun BooleanQuery.Builder.addLongQuery(fieldName: String, termValue: PrimaryKey) {
        add(LongPoint.newExactQuery(fieldName, termValue), BooleanClause.Occur.MUST_NOT)
    }

    private fun BooleanQuery.Builder.addMustLongQuery(fieldName: String, termValue: PrimaryKey) {
