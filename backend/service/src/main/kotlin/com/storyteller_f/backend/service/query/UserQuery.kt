...
import com.storyteller_f.backend.service.isEmpty
...
import com.storyteller_f.backend.service.tables.AlternateAccounts
import com.storyteller_f.backend.service.tables.AlternateAccounts.hostId
...
import com.storyteller_f.backend.service.tables.mapUserInfo
...
suspend fun ExposedDatabaseSession.getUserRawResultAndPublicKeyByAddress(
    ...
    first {
        val value = User.Companion.wrapRow(it)
    ...
    listOf({
        ...
        if (!name.isNullOrBlank() || avatar != null) {
            Users.update({
                ...
                if (avatar != null) {
                    it[Users.icon] = avatar.ifEmpty { null }
                }
            } > 0
        } else {
            true
